...
class PFCropView: UIView, UIScrollViewDelegate, UIGestureRecognizerDelegate, PFCropRectViewDelegate {
    ...
    private let cropRectView = PFCropRectView()
    ...
    private var interfaceOrientation = UIApplication.shared.statusBarOrientation
    ...
    var image: UIImage? {
        didSet {
            if image != nil, let newImage = image {
    ...
    var croppedImage: UIImage? {
        image?.rotatedImageWithTransform(rotation, croppedToRect: zoomedCropRect())
    }
    ...
    private func initializeCropView() {
        ...
        rotationGestureRecognizer = UIRotationGestureRecognizer(target: self, action: #selector(PFCropView.handleRotation(_:)))
    ...
    private func showOverlayView(_ shouldShow: Bool) {
        let color = shouldShow ? UIColor(white: 0.0, alpha: 0.4) : UIColor.clear
    ...
    private func setupEditingRectArea() {
        let interfaceOrientation = UIApplication.shared.statusBarOrientation
        if interfaceOrientation.isPortrait {
    ...
    private func cappedCropRectInImageRectWithCropRectView(_ cropRectView: PFCropRectView) -> CGRect {
...
extension UIImage {
    ...
    func rotatedImageWithTransform(_ transform: CGAffineTransform) -> UIImage {
