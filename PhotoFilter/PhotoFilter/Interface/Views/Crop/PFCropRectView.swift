...
//  PhotoFilter
//
//  Created by Juhi Sinha.
//  Inspired by: https://github.com/sprint84/PhotoCropEditor

import Foundation
import UIKit

protocol PFCropRectViewDelegate: class {
    func cropRectViewDidBeginEditing(_ view: PFCropRectView)
    func cropRectViewDidChange(_ view: PFCropRectView)
    func cropRectViewDidEndEditing(_ view: PFCropRectView)
}

class PFCropRectView: UIView, ResizeControlDelegate {
    weak var delegate: PFCropRectViewDelegate?
    private var resizeImageView: UIImageView!
    private let topLeftCornerView = PFResizeControl()
    private let topRightCornerView = PFResizeControl()
    private let bottomLeftCornerView = PFResizeControl()
    private let bottomRightCornerView = PFResizeControl()
    private let topEdgeView = PFResizeControl()
    private let leftEdgeView = PFResizeControl()
    private let rightEdgeView = PFResizeControl()
    private let bottomEdgeView = PFResizeControl()
    ...
    private func initializeView() {
        ...
        let image = UIImage(named: "crop_boundary", in: bundle, compatibleWith: nil)
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        for subview in subviews where subview is PFResizeControl {
    ...
    func resizeControlDidBeginResizing(_ control: PFResizeControl) {
        initialRect = frame
        delegate?.cropRectViewDidBeginEditing(self)
    }
    
    func resizeControlDidResize(_ control: PFResizeControl) {
        frame = cropRectWithResizeControlView(control)
        delegate?.cropRectViewDidChange(self)
    }
    
    func resizeControlDidEndResizing(_ control: PFResizeControl) {
        delegate?.cropRectViewDidEndEditing(self)
    }
    
    private func cropRectWithResizeControlView(_ resizeControl: PFResizeControl) -> CGRect {
        var rect = frame
    ...
    private func constrainedRectWithRectBasisOfWidth(_ frame: CGRect) -> CGRect {
        var result = frame
    ...
    private func constrainedRectWithRectBasisOfHeight(_ frame: CGRect) -> CGRect {
        var result = frame
