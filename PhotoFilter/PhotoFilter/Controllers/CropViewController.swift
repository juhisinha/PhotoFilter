...
protocol CropViewControllerDelegate: AnyObject {
 ...
 class CropViewController: UIViewController {
     ...
     var zoomedCropRect: CGRect {
        cropView!.zoomedCropRect()
    }

    private var cropView: PFCropView?
    ...
    private func initialize() {
     ...
     override func loadView() {
        ...
        // Add CropView
        cropView = PFCropView(frame: contentView.bounds)
    ...
    @objc func cancel(_ sender: UIBarButtonItem) {
        delegate?.cropViewControllerCancel(self)
    }
    ...
    @objc func done(_ sender: UIBarButtonItem) {
        if let image = cropView?.croppedImage {
            ...
            delegate?.cropViewDidFinishCropping(self, didFinishCroppingImage: image, transform: rotation, cropRect: rect)
        }
    }
